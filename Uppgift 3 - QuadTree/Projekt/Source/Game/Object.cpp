...
void Object::Init(const Tga2D::Vector2f& aPos, const Tga2D::Vector2f& aDim)
{
	...
	mySprite->SetSizeRelativeToScreen({ 1,1 });
	mySprite->SetPivot({ 0,0 });
	myColor = { 1,1,1,0.5f };
}
...
void Object::Render()
{
	...
	mySprite->SetSizeRelativeToScreen({ 10.f / Tga2D::CEngine::GetInstance()->GetWindowSize().myX * Tga2D::CEngine::GetInstance()->GetWindowRatio(), 10.f / Tga2D::CEngine::GetInstance()->GetWindowSize().myY });
	mySprite->Render();
}
